...
function detectarViolacoesCs(caminho, conteudo) {
  const violacoes = [];
...
function detectarViolacoesTs(conteudo) {
  const violacoes = [];
...
function detectarViolacoesVue(conteudo) {
  const violacoes = [];
...
function detectarViolacoesStore(caminho, conteudo) {
  if (/(loading|carregando)\s*=\s*(false|true)/.test(conteudo) && !/finally/.test(conteudo))
    return ["AVISO: Loading sem finally. Reset no finally."];
  return [];
}

export function validarArquivo(caminho, conteudo) {
  const ext = caminho.split(".").pop()?.toLowerCase();
  if (ext !== "cs" && ext !== "vue" && ext !== "ts") return [];

  const violacoes = [
    ...detectarComentarios(conteudo),
    ...detectarIfComChaves(conteudo),
    ...detectarIfElseDesnecessario(conteudo)
  ];

  if (ext === "cs") violacoes.push(...detectarViolacoesCs(caminho, conteudo));
  if (ext === "vue" || ext === "ts") violacoes.push(...detectarViolacoesTs(conteudo));
  if (ext === "vue") violacoes.push(...detectarViolacoesVue(conteudo));
  if (ext === "ts" && /Store|store/.test(caminho)) violacoes.push(...detectarViolacoesStore(caminho, conteudo));
