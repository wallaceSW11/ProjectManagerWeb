...
class RepositoriosService extends BaseApiService {
  ...
  async adicionarRepositorio(repositorio: IRepositorio): Promise<void> {
    const dto = (repositorio as any).toDTO
      ? (repositorio as any).toDTO()
      : repositorio;
  ...
  async atualizarRepositorio(repositorio: IRepositorio): Promise<void> {
    const dto = (repositorio as any).toDTO
      ? (repositorio as any).toDTO()
      : repositorio;
    return await this.put(`repositorios/${repositorio.identificador}`, dto);
  }
  ...
  async atualizarOrdem(
    indices: { identificador: string; indice: number }[]
  ): Promise<void> {
    return await this.put('repositorios/indices', indices);
  }

  async adicionarCodigoTarefa(
    repositorioId: string,
    codigoTarefa: ICodigoTarefa
  ): Promise<void> {
    return await this.post(
      `repositorios/${repositorioId}/codigos-tarefa`,
      codigoTarefa
    );
  }

  async atualizarCodigoTarefa(
    repositorioId: string,
    codigoId: string,
    codigoTarefa: ICodigoTarefa
  ): Promise<void> {
    return await this.put(
      `repositorios/${repositorioId}/codigos-tarefa/${codigoId}`,
      codigoTarefa
    );
  }

  async removerCodigoTarefa(
    repositorioId: string,
    codigoId: string
  ): Promise<void> {
  ...
  async buscarCodigoTarefa(
    iniciais: string
  ): Promise<{ codigoTarefa: ICodigoTarefa; repositorio: any } | null> {
    try {
      return await this.get(
        `repositorios/codigos-tarefa/${encodeURIComponent(iniciais)}`
      );
    } catch {
      return null;
    }
